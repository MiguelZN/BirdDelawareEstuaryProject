...
public class Animation {
	...
	public Animation(String sprite_sheet_path, Size s, int num_rows, int num_columns, int num_frames, int fps) {
		 ...
		 * Thus we do 60fps/1fps (1 frame per second) = 60 tick multiplier
		 * 1000ms/60fps = 16.66ms. 16.66ms * 60 tick multipler = 1000ms threshold.
		 * 
		 * Thus every 
		 * */
...
//	}
	...
	public void updateImgSrcXY() {
		...
		System.out.printf("X1:%d, Y1:%d, X2:%d, Y2:%d, INDEX:%d\n", this.imgsrcx1, this.imgsrcy1, this.imgsrcx2, this.imgsrcy2, this.frame_index);
		}
	}
...
//	}
	...
	public void update() {
		/*System.out.println("UPDATING TICK");
		System.out.println(this.current_tick);
	...
	public void drawImageExact(Graphics g, Position p, Size draw_size) {
		...
		g.drawImage(this.spritesheet, destx1, desty1, destx2, desty2, this.imgsrcx1, this.imgsrcy1, this.imgsrcx2, this.imgsrcy2, null, null);
		this.update();
	}
